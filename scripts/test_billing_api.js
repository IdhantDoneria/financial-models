...
async function buy(token, plan, period = "monthly") {
  const order = await call(handlers.billingOrder, { method: "POST", token, body: { plan, period } });
...
(async () => {
  ...
  const badPeriod = await call(handlers.billingOrder,
  ...
  check(`usage: PRO unblocks uploads (${FREE_LIMIT + 1}/50)`,
    proUse.code === 200 && proUse.body.plan === "pro" && proUse.body.limit === 50
    && proUse.body.used === FREE_LIMIT + 1);

  // -- upgrade to UNLIMITED (monthly) w/ day carry-over --------------------
  const up = await buy(token, "unlimited", "monthly");
  ...
  const annualPurchase = await buy(annualToken, "pro", "annual");
  check("purchase: ANNUAL pass is ~365 days, not 30",
    annualPurchase.body.ok
    && Date.parse(annualPurchase.body.subscription.expiresAt) - Date.now() > 360 * 86_400_000);

  // -- webhook fallback (buyer's tab died before verify) -------------------
  const whToken = await login("webhook-buyer@example.com");
  const whOrder = await call(handlers.billingOrder,
    { method: "POST", token: whToken, body: { plan: "pro", period: "monthly" } });
  const whBody = JSON.stringify({
    event: "payment.captured",
    payload: { payment: { entity: { id: "pay_wh1", order_id: whOrder.body.orderId,
      notes: { email: "webhook-buyer@example.com", plan: "pro", period: "monthly" } } } },
  });
  ...
  check(`expiry: lapsed PRO falls back to FREE ${FREE_LIMIT}/mo`,
    lapsed.body.plan === "free" && lapsed.body.limit === FREE_LIMIT);
